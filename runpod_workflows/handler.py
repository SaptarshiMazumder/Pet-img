"""
RunPod Serverless Handler — multi-workflow ComfyUI runner.

Supports two workflows selected via the `workflow_type` input field:

  "zturbo"  (default) — ZTurboLoraPet: z_image_turbo + wetInk/ukiyoe LoRAs
  "uso"               — Flux1-Dev USO: reference-image style transfer via USO
...
# Paths & config
# ---------------------------------------------------------------------------
WORKFLOW_ZTURBO_PATH = "/workflow_api.json"
WORKFLOW_USO_PATH    = "/workflow_api_uso.json"
...
def upload_image_to_comfyui(image_bytes: bytes, filename: str) -> str:
    """Upload an image to ComfyUI's input directory. Returns the filename ComfyUI assigned."""
    resp = requests.post(
...
# Workflow builders
# ---------------------------------------------------------------------------
def build_zturbo_workflow(job_input: dict) -> dict:
    with open(WORKFLOW_ZTURBO_PATH) as f:
        wf = json.load(f)
...
def build_uso_workflow(job_input: dict) -> dict:
    with open(WORKFLOW_USO_PATH) as f:
        wf = json.load(f)

    if "prompt" in job_input:
        wf["130"]["inputs"]["text"] = str(job_input["prompt"])

    if "width" in job_input:
        wf["131"]["inputs"]["width"] = int(job_input["width"])
    if "height" in job_input:
        wf["131"]["inputs"]["height"] = int(job_input["height"])
    if "batch_size" in job_input:
        wf["131"]["inputs"]["batch_size"] = int(job_input["batch_size"])

    if "steps" in job_input:
        wf["135"]["inputs"]["steps"] = int(job_input["steps"])
    if "cfg" in job_input:
        wf["135"]["inputs"]["cfg"] = float(job_input["cfg"])
    if "guidance" in job_input:
        wf["144"]["inputs"]["guidance"] = float(job_input["guidance"])

    seed = job_input.get("seed", -1)
    if seed is None or int(seed) < 0:
        seed = random.randint(0, 2**32 - 1)
    wf["135"]["inputs"]["seed"] = int(seed)
    wf["135"]["inputs"]["control_after_generate"] = "fixed"

    if "lora_strength" in job_input:
        wf["128"]["inputs"]["strength_model"] = float(job_input["lora_strength"])
    if "lora_name" in job_input:
        wf["128"]["inputs"]["lora_name"] = str(job_input["lora_name"])

    # Upload reference images to ComfyUI if URLs are provided
    if "style_image_1" in job_input:
        img_bytes = fetch_image_bytes(job_input["style_image_1"])
        name = upload_image_to_comfyui(img_bytes, "style_ref_1.png")
        wf["129"]["inputs"]["image"] = name

    if "subject_image" in job_input:
        img_bytes = fetch_image_bytes(job_input["subject_image"])
        name = upload_image_to_comfyui(img_bytes, "subject_ref.png")
        wf["141"]["inputs"]["image"] = name
...
def handler(job: dict) -> dict:
    ...
    workflow_type = job_input.get("workflow_type", "zturbo").lower()

    try:
        if workflow_type == "uso":
            workflow = build_uso_workflow(job_input)
            seed_node = "135"
        else:
            workflow = build_zturbo_workflow(job_input)
            seed_node = "31"
    ...
    try:
        ...
        print(f"[job] queued prompt {prompt_id} (workflow={workflow_type})")
    ...
    seed = workflow[seed_node]["inputs"]["seed"]
    uploaded = []
    ...
    if workflow_type == "zturbo":
        result["upscale_denoise"] = workflow["38"]["inputs"]["denoise"]
        result["upscale_sampler"] = workflow["38"]["inputs"]["sampler_name"]
        result["upscale_scheduler"] = workflow["38"]["inputs"]["scheduler"]
